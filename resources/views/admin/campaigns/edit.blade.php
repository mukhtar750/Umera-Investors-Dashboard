...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-skin-text">Edit Campaign</h1>
    ...
    <div class="bg-skin-base shadow-sm rounded-lg border border-skin-border p-6">
        <form action="{{ route('admin.campaigns.update', $campaign) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="subject" id="subject" value="{{ old('subject', $campaign->subject) }}" required class="mt-1 block w-full border-skin-border rounded-md shadow-sm focus:ring-umera-500 focus:border-umera-500 sm:text-sm bg-skin-fill text-skin-text">
            </div>
            ...
            <div>
                ...
                <select name="audience_ids[]" id="audience_ids" multiple required class="mt-1 block w-full border-skin-border rounded-md shadow-sm focus:ring-umera-500 focus:border-umera-500 sm:text-sm bg-skin-fill text-skin-text h-32">
                    @foreach($audiences as $audience)
                        <option value="{{ $audience->id }}" {{ $campaign->audiences->contains($audience->id) ? 'selected' : '' }}>
                            {{ $audience->name }} ({{ $audience->contacts()->count() }} contacts)
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
                    ...
                    <label class="relative cursor-pointer group">
                        <input type="radio" name="template" value="{{ $template }}" class="peer sr-only" required {{ old('template', $campaign->template) === $template ? 'checked' : '' }}>
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="10" required class="mt-1 block w-full border-skin-border rounded-md shadow-sm focus:ring-umera-500 focus:border-umera-500 sm:text-sm bg-skin-fill text-skin-text font-mono">{{ old('content', $campaign->content) }}</textarea>
