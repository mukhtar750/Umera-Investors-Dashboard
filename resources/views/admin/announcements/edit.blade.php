...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-skin-text">Edit Announcement</h1>
        ...
        <form action="{{ route('admin.announcements.update', $announcement) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="6" required
                    class="mt-1 block w-full rounded-md border-skin-border shadow-sm focus:border-umera-500 focus:ring-umera-500 sm:text-sm bg-skin-fill text-skin-text">{{ old('content', $announcement->content) }}</textarea>
            </div>

            <!-- Published -->
            <div class="flex items-start">
                <div class="flex items-center h-5">
                    <input id="is_published" name="is_published" type="checkbox" value="1" {{ old('is_published', $announcement->is_published) ? 'checked' : '' }}
